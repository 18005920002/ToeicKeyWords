...
| 功能 | 说明 |
| --- | --- |
| 按单元浏览 | 内置 30 个单元（Unit 1–30，每单元 Part A/B 各 20 词，共 1200 词），侧栏显示每单元词汇量与掌握进度；另有「🔎 全部单元」一次浏览全部。单元导航默认收起，点顶部「📚」展开（手机上是覆盖式抽屉，选完单元自动收起） |
| 词汇卡片 | 每张卡片同屏展示全部字段：词条、音标、词性、英文注释、中文注释、例句 + 译文（高度随内容自适应，长例句完整显示） |
| 检索 | 顶部「🔍」展开搜索框，跨全部单元检索单词 / 释义 / 例句；结果卡片带「Unit X · Part A/B」徽标，便于定位出处。收起时自动清掉检索词与筛选条件，收起后若仍有条件生效，「🔍」上会亮小圆点 |
| 例句选词 | 长按或选中例句里的任意单词，点「＋ Unit31 扩展词」就把它收进第 31 单元：词条为选中的单词，例句沿用当前句（含中文注释），音标 / 读音 / 释义只占位，后续人工补齐 |
...
阅读例句时遇到没在词书里的词，可以直接抠出来收藏：

- 触发：长按例句中的单词（会自动扩到整词），或用鼠标 / 手指拖选一个单词，浮层就在选区上方出现
- 词条为选中的单词；例句沿用当前这一句并带中文注释，另存一行「摘自 Unit X · 原词条」方便日后核对上下文
- 音标、词性、英文 / 中文注释、预生成读音都只展示占位符（卡片上带「待补充」徽标），点 🔊 仍能用本机合成语音先听一发
- 该单元以自定义词库的形式存在 `localStorage`（id `custom-unit-31-extension`），同样走「导出 / 导入 JSON」备份，侧栏点 ✕ 可整个删除；重复收录同一个词会提示而不重复写入
- 人工补齐方式：先导出备份，把该词的 `pending` 改为 `false` 并填上各字段，再导入覆盖（`pending` 为 `false` 后占位符就换成正常展示）
...
递增 `sw.js` 顶部的 `CACHE_VERSION`（当前 `toeic-v4`）并重新部署。手机下次打开时，新 Service Worker 会停在 waiting，
